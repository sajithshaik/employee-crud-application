...
function EditEmployee() {
  ...
  const createUsers = async () => {
    const result = await axios.put(`http://localhost:3200/users/${id}`, formValues);
      setIsSubmit(true);
      console.log(result);
    ...
    const getUser = async () => {
        const result = await axios.get(`http://localhost:3200/users/${id}`);
         setFormValues({name: result.data.name, email: result.data.email, username: result.data.username})
          console.log(result);
